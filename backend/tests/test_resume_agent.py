...
class TestResumeStructurer:
    ...
    async def test_output_schema_keys(self):
        ...
        result = await resume_structurer("some text")
        assert set(result.keys()) == {"structured_resume", "evaluation"}

    async def test_structured_resume_schema(self):
    ...
    async def test_evaluation_schema(self):
    ...
    async def test_completeness_score_range(self):
        """完整度评分在 0-100 范围内。"""
        result = await resume_structurer("some text")
        assert 0 <= result["evaluation"]["completeness_score"] <= 100

    async def test_fallback_extracts_phone_and_email(self):
        """fallback 模式提取电话和邮箱。"""
        text = (
            "姓名：张三\n电话：13800138000\n邮箱：test@example.com\n技能：Python、SQL"
        )
        result = await resume_structurer(text, llm=None)
    ...
    async def test_fallback_extracts_explicit_technologies_without_skills_heading(self):
    ...
    async def test_fallback_extracts_pipe_style_pdf_resume(self):
    ...
    async def test_fallback_completeness_score(self):
    ...
    async def test_fallback_missing_sections(self):
        """fallback 模式检测缺失的部分。"""
        text = "姓名：张三\n电话：13800138000"
        result = await resume_structurer(text, llm=None)
    ...
    async def test_injected_llm_dict_response(self):
        ...
        result = await resume_structurer("姓名：张三", llm=fake_llm)
        assert result["structured_resume"]["basic_info"]["name"] == "张三"
        assert result["evaluation"]["completeness_score"] == 88
        assert "项目清晰" in result["evaluation"]["strengths"]

    async def test_injected_llm_json_string_response(self):
        ...
        result = await resume_structurer("姓名：李四", llm=fake_llm)
        assert result["structured_resume"]["basic_info"]["name"] == "李四"
        assert result["evaluation"]["completeness_score"] == 50

    async def test_injected_llm_markdown_wrapped_response(self):
        ...
        result = await resume_structurer("姓名：王五", llm=fake_llm)
        assert result["structured_resume"]["basic_info"]["name"] == "王五"

    async def test_injected_llm_score_clamped(self):
        ...
        result = await resume_structurer("text", llm=fake_llm)
        assert result["evaluation"]["completeness_score"] == 100

    async def test_configured_llm_path_uses_model_adapter(self, monkeypatch):
        """启用已配置模型时应调用模型适配器，而不是发生名称解析错误。"""

        def fake_configured_llm(prompt: str):
        ...
        monkeypatch.setattr(
            "app.tools.resume_structurer.configured_llm", fake_configured_llm
        )

        result = await resume_structurer("姓名：赵六", use_configured_llm=True)
    ...
    async def test_build_prompt_contains_schema(self):
...
class TestResumeAnalysisAgentNodes:
    ...
    async def test_detect_file_type_node(self, tmp_path):
    ...
    async def test_convert_pdf_node_skips_docx(self, tmp_path):
    ...
    async def test_convert_pdf_node_unknown_type(self):
    ...
    async def test_extract_text_node(self, tmp_path):
    ...
    async def test_extract_text_node_reads_original_pdf(self, tmp_path, monkeypatch):
    ...
    async def test_structure_resume_node(self):
        ...
        await structure_resume_node(state, use_configured_llm=False)
...
class TestResumeAnalysisAgentRun:
    ...
    async def test_run_docx_full_pipeline(self, tmp_path):
        ...
        final_state = await run_resume_analysis_agent(state, use_configured_llm=False)
    ...
    async def test_run_unknown_file_type_sets_error(self, tmp_path):
        """不支持的文件类型设置 error 状态。"""
        txt = tmp_path / "resume.txt"
        txt.write_text("hello", encoding="utf-8")
    ...
    async def test_run_doc_file_type_sets_error(self, tmp_path):
        """.doc 文件类型设置 error 状态。"""
        doc = tmp_path / "resume.doc"
        doc.write_bytes(b"\xd0\xcf\x11\xe0\xa1\xb1\x1a\xe1body")
    ...
    async def test_state_callback_invoked(self, tmp_path):
        ...
        await run_resume_analysis_agent(
    ...
    async def test_run_with_injected_llm(self, tmp_path):
        ...
        final_state = await run_resume_analysis_agent(state, llm=fake_llm)
    ...
    async def test_agent_class_directly(self, tmp_path):
